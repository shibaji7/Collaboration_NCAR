...
function [axis_handle, ray_handle] = plot_ray_iono_slice_doppler(iono_grid, ...
      start_range, end_range, range_inc, start_height, end_height, ...
      height_inc, ray, dfs, varargin)
...
max(iono_grid(:)),min(iono_grid(:))
handle = pcolor(iono_X, iono_Y, iono_grid);
%handle = pcolor(iono_X, iono_Y, dfs(1,:,:)');
shading flat
axis equal
