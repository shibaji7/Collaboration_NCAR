...
%
% Purpose :
%   Compares the spherical 2D NRT and 3D (no magnetic field) NRT for a fan of 
%   rays
%
% Calling sequence :
%   NRT_sp_comparison
...
en_profile(en_profile < 0) = 0;

% make start of ionosphere smoother - try turing this off
plasma = sqrt(en_profile);
idx = find(plasma ~= 0);
im = min(idx);
plasma(im-1) = plasma(im) ./ 2;
plasma(im-2) = plasma(im) ./ 4;
plasma(im-3) = plasma(im) ./ 8;
plasma(im-4) = plasma(im) ./ 16;
plasma(im-5) = plasma(im) ./ 32;
plasma(im-6) = plasma(im) ./ 64;
en_profile = plasma.^2;
...
[ray_data, ray_path_data] = ...
    raytrace_2d_sp(elevs, ray_bears(1), freqs, nhops, tol, ...
                   rad_earth/1000, irregs_flag, iono_en_grid_2D, ...
		   iono_en_grid_5_2D, collision_freq_2D, ht_start, ht_inc, ...
		   range_inc, irreg);
toc	    
