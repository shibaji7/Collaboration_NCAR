...
save(['../' dic 'exp.bgc.bm(' num2str(bm) ').ne.mat'], 'iono_en_grid')
...
for i=1:num_elevs
	...
	writetable(T, strrep(fbgc, '<elv>', num2str(elevs(i))));
end

cd pharlap/;
iono_en_grid(d_start:d_end,:) = iono_en_grid(d_start:d_end,:) * d_ratio;
iono_en_grid(f_start:f_end,:) = iono_en_grid(f_start:f_end,:) * f_ratio;
save(['../' dic 'exp.flare.bm(' num2str(bm) ').ne.mat'], 'iono_en_grid')
iono_en_grid_5(d_start:d_end,:) = iono_en_grid_5(d_start:d_end,:) * d_ratio;
iono_en_grid_5(f_start:f_end,:) = iono_en_grid_5(f_start:f_end,:) * f_ratio;
...
for i=1:num_elevs
	...
	T = array2table(X);
	T.Properties.VariableNames(1:2) = {'height','grange'};
	writetable(T, strrep(fflare, '<elv>', num2str(elevs(i))));
end
toc
