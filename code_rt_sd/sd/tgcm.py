...
class TGCM(object):
    ...
    def _interpolate_(self, i):
        ...
        if not os.path.exists(fname):
            ...
            for i, la, lo in zip(range(len(self.lat)), self.lat, self.lon):
                _ij_ = _get_ij_(latx, lonx, la, lo)
                ne[:, i] = nex[:, _ij_[0], _ij_[1]]
            m["ne"] = ne
            savemat(fname, m)
    ...
    def _exe_(self):
        ...
        for i in range(self.tsim_start, self.tsim_end):
            ...
            if self.verbose: self._compute_(i)
            plotlib.plot_rays(dic, self.start + dt.timedelta(minutes=i), i, self.bmnum)
        if self.verbose: print("\n Interpolation completed.")
        if self.verbose: print("\n Processing Doppler.")
        #for i in range(self.tsim_start + dt.timedelta(minute=1), self.tsim_end):
        #self._compute_doppler_(7,0)
        #    break
        return
