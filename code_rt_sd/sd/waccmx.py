...
class WACCM(object):
    ...
    def _estimate_bearing_(self):
        ...
        fname = "data/sim/{dn}/{rad}/bearing.mat".format(dn=self.event.strftime("%Y.%m.%d.%H.%M"), 
                rad=self.rad)
        m = {}
        lat, lon, bearing = utils.get_sd_radar(self.rad)
        bearing = (self.bmnum - 12)*2.3
        p = (lat, lon)
    ...
    def _compute_doppler_(self, i):
        ...
        for fi in fnf:
            ...
            print(90.-elvi, np.cos(np.deg2rad(90.-elvi)))
    ...
    def _exe_(self):
        ...
        x.to_csv("data/sim/{dn}/{rad}/velocity.bm({bm}).ts.csv".format(dn=self.event.strftime("%Y.%m.%d.%H.%M"), 
            rad=self.rad, bm=self.bmnum), index=False)
    ...
    def _arch_(self):
        ...
        os.system("cp -r data/sim/{dn}/{rad}/* {arc}".format(dn=self.event.strftime("%Y.%m.%d.%H.%M"), 
            rad=self.rad, arc=arc))
        cmd = "ffmpeg -r 1 -i {arc}/waccmx_ti\(%d\).bm({bm}).png -c:v libx264 -vf \
                'scale=1420:-2,fps=3,format=yuv420p' {arc}/waccmx_edens_rad.bm({bm}).mp4".format(arc=arc, bm=self.bmnum)
        os.system(cmd)
        cmd = "ffmpeg -r 1 -i {arc}/rt.ti\(%d\).bm\({bm}\).d.png -c:v libx264 -vf \
                'scale=1420:-2,fps=3,format=yuv420p' {arc}/waccmx_rt_daily.bm({bm}).mp4".format(arc=arc, bm=self.bmnum)
        os.system(cmd)
        cmd = "ffmpeg -r 1 -i {arc}/rt.ti\(%d\).bm\({bm}\).f.png -c:v libx264 -vf \
                'scale=1420:-2,fps=3,format=yuv420p' {arc}/waccmx_rt_flare.bm({bm}).mp4".format(arc=arc, bm=self.bmnum)
        os.system(cmd)
