...
prompt={'origin_lat','origin_long','ray_bear','end_range','freq_start','freq_inc','freq_end','elev_start','elev_inc','elev_end'};
dlg_title='Input Values';
num_lines=1;
defAns={origin_lat,origin_long,ray_bear,end_range,freq_start,freq_inc,freq_end,elev_start,elev_inc,elev_end};
options='on';
% Take inputs
...
mid_idx=1;
% mid_idx=fix((end_range_idx + start_range_idx));
start_ht = start_height;
start_ht_idx = 1;

% damping_factor=ones(end_range)
...
for i=1:size(UT_array,1)
    ...
    for m=1:5
        ...
        for s=1:num_range
            iono_pf_grid(:, s)= iono_pf_grid(:, s) * damping_factor(m,s);
        end
        ...
        for freq=freq_start:freq_inc:freq_end
            ...
            UT_str = [num2str(UT(3)) '-' num2str(UT(2)) '-' num2str(UT(1)) '_' ...
