...
% setup general stuff
%
function NoEclipse_Raytracing(origin_lat_p,origin_long_p,ray_bear_p, UT_Array_n, dir_path)
...
idx = 1;
start_range = 0;
%start_range = start_range+offset;
...
for i=1:size(UT_array,1)
    ...
    for freq=freq_start:freq_inc:freq_end
        ...
        title(fig_str)
        figname=strcat(freq_str,'-',UT_str,'.png');
		t= annotation('textbox',[.43 .52 .4 .5],'String', dir_path);
        t.FontSize = 12;
        t.LineStyle = 'none';
        %saveas(gcf,figname);
		%F.set(mTextBox,'String',dir_path);
		set(gcf, 'PaperPositionMode','auto'); 
		saveas(gcf,[dir_path '\NoEclipse_' fig_str '.png']);
        cnt=cnt+1;
    end
end
end