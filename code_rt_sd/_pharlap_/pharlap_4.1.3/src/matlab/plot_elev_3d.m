% script for comparing raytrace plots of normal and eclipsed ionosphere for fixed azimuth,
% varing elevations
close all
cnt = 1;

for freq=freq_start:freq_inc:freq_end
    freqs = freq.*ones(size(elevs_a)); % frequency (MHz)
...
[ray_data_N, ray_N, ray_state_vec_N] = ...
  raytrace_3d(origin_lat, origin_long, origin_ht, elevs_a, ray_bears_a, freqs, ...
...
hold off
grid on
title([UT_str ', ' freq_str ', azimuth:' num2str(ray_bears_a(1))]);
xlabel('latitude (deg)')
