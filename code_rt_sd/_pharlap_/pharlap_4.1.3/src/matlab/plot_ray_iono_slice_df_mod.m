...
function [axis_handle, ray_handle] = plot_ray_iono_slice_df_mod(iono_grid, ...
...
% if using softeare opengl then limit the size of the window to avoid
...
for idx = 1:length(ray)
  ...
  if ~isempty(ray(idx).gndrng)
    ...
    ray_gndrng = [ray(idx).gndrng(1) : 1 : ray(idx).gndrng(len)];
    ...
    %cc = df'/max(df);
    cc = df'*1e6;
    size(cc),size(ray_X),size(ray_Y)
    %plot_cmd = 'h = plot(ray_X, ray_Y, ''color'',[1,0,0.99],';    
    %plot_cmd = [plot_cmd '''linewidth'',[2]);'];
    
    %for k = 1:length(ray_X)
    %    rx = ray_X(k);
    %    ry = ray_Y(k);
    %    plot(rx,ry,'color',[1,0,cc(k)],'','linewidth',2)
    %    hold on
    %end
    scatter(ray_X,ray_Y,[],cc)
